...
class ModuleView(generics.CreateAPIView):
    ...
    def get(self, request):
       ...
       # is_top_domain_user = request.user.domain_user_id and request.user.domain_user_id.id == request.user.id
        
        is_top_domain_user  = (
            request.user.domain_user_id is None
            or request.user.domain_user_id.id == request.user.id
        )
       
        if is_super_admin or is_top_domain_user:
            menus = Modules.objects.filter(is_menu=True, parent_id=None, is_active=True).order_by('display_order')
        else:
            permission_module_ids = UserPermissions.objects.filter(user=request.user.id, is_permission=True).values_list('module_id', flat=True)
            menus = Modules.objects.filter(is_menu=True, parent_id=None, is_active=True).filter(id__in=permission_module_ids).order_by('display_order')
        ...
        for menu in serialized_menus:
            ...
            if is_super_admin or is_top_domain_user:
                menu['fields']['submenus'] = Modules.objects.filter(
                    ...
                    'id', 'module_name', 'module_icon', 'is_menu', 'is_active',
                    'parent_id', 'display_order', 'module_url', 'module_description')
            else:
                menu['fields']['submenus'] = Modules.objects.filter(
                    parent_id=menu['pk'], is_active=True, is_menu=True
                ).filter(id__in=permission_module_ids).order_by('display_order').values(
                    'id', 'module_name', 'module_icon', 'is_menu', 'is_active',
                    'parent_id', 'display_order', 'module_url', 'module_description')
        ...
        if is_super_admin:
            cleaned_menus.append({
                'id': 0,
                'module_name': 'Manage Module Urls',
                'module_icon': '',
                'is_menu': True,
                'is_active': True,
                'parent_id': None,
...
class ModuleUrlsListAPIView(APIView):
    ...
    def get(self, request):
        ...
        return renderResponse(
            data={'moduleUrls': urlJson, 'project_urls': urlsProject, 'modules': modulesJson},
    ...
    def post(self, request):
        ...
        for item in data:
            if item['url'] is not None:
                ...
                if item['module'] != 0 and item['module'] is not None:
                    moduleUrls.module = Modules.objects.get(id=item['module'])
                moduleUrls.save()
